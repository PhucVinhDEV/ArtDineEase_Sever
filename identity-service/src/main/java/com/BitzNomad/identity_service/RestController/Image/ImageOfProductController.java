...
public class ImageOfProductController {
    ...
    public ApiResponse<Set<ImageDTOReponese>> uploadImage(@RequestParam("file") MultipartFile[] file, @RequestParam("productID") Long productID,
                                                          @RequestParam("typeOfImg") String typeOfImg) throws Exception {

        return ApiResponse.<Set<ImageDTOReponese>>builder()
                .status(201)
                .result(imageOfProductService.saveImageOfProduct(file,productID,typeOfImg))
                .build();
    }
