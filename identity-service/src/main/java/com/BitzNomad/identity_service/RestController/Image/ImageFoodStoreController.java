...
public class ImageFoodStoreController {
    ...
    public ApiResponse<Set<ImageDTOReponese>> uploadImage(@RequestParam("file") MultipartFile[] file, @RequestParam("foodstoreID") Long restaurantID
                                                          ) throws Exception {

        return ApiResponse.<Set<ImageDTOReponese>>builder()
                .status(201)
                .result(imageOfFoodStoreService.saveImageOfFoodStore(file,restaurantID,PredefineImageOfrestaurant.MAIN_IMAGE))
                .build();
    }

    @PostMapping("/banner")
    @Operation(summary = "Upload a banner image", description = "Uploads a banner image file to Cloudinary and returns the upload result. (Test PostMan nha)")
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Banner image uploaded successfully", content = @Content(mediaType = "application/json")),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "500", description = "Internal server error", content = @Content(mediaType = "application/json"))
    })
    public ApiResponse<Set<ImageDTOReponese>> uploadBannerImage(@RequestParam("file") MultipartFile[] file,
                                                                @RequestParam("foodstoreID") Long restaurantID) throws Exception {


        return ApiResponse.<Set<ImageDTOReponese>>builder()
                .status(201)
                .result(imageOfFoodStoreService.saveImageOfFoodStore(file, restaurantID, PredefineImageOfrestaurant.BANNER_IMAGE))
                .build();
    }
    @DeleteMapping
